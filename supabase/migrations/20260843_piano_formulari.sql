-- 20260843_piano_formulari.sql
-- Archivio formulari della sezione Piano: PDF/Word/Excel (max 2 MB, base64
-- come hr_allegati), organizzati in cartelle nominabili, per settore.

CREATE TABLE IF NOT EXISTS piano_formulari (
  id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  nome TEXT NOT NULL,
  cartella TEXT DEFAULT 'Generale',
  mime TEXT,
  estensione TEXT,
  dimensione INT,
...
CREATE INDEX IF NOT EXISTS idx_piano_formulari_rep ON piano_formulari (reparto_dip, cartella);

ALTER TABLE piano_formulari ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS deny_all_anon ON piano_formulari;
CREATE POLICY deny_all_anon ON piano_formulari FOR ALL TO anon USING (false) WITH CHECK (false);
...
BEGIN
  ...
  IF p_table NOT IN (
    ...
    'piano', 'piano_turni', 'piano_codici', 'piano_fabbisogni', 'piano_regole', 'piano_festivi', 'piano_timbrature', 'piano_mappature', 'piano_vacanze', 'piano_regole_gruppo', 'piano_formulari'
  ) THEN
