-- 20260841_piano_vacanze.sql
-- Sezione Vacanze come Turnivo: settimane ISO per collaboratore per anno.
-- + colonna is_jolly (per i congedi C: 2 prima per i jolly, 1 per i fissi)
-- + codice speciale WD (diurno forzato prima delle vacanze, dal solver Turnivo)
-- + whitelist secure_* aggiornata con piano_vacanze

CREATE TABLE IF NOT EXISTS piano_vacanze (
  id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  collaboratore TEXT NOT NULL,
  settimana INT NOT NULL CHECK (settimana BETWEEN 1 AND 53),
  anno INT NOT NULL,
  confermata BOOLEAN DEFAULT FALSE,
  operatore TEXT,
  created_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE (collaboratore, settimana, anno)
);
...
BEGIN
  ...
  IF p_table NOT IN (
    ...
    'piano', 'piano_turni', 'piano_codici', 'piano_fabbisogni', 'piano_regole', 'piano_festivi', 'piano_timbrature', 'piano_mappature', 'piano_vacanze'
  ) THEN
