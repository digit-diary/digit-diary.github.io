-- ============================================================
-- STORICO HR (riservato admin + operatori selezionati)
-- 1) collaboratori.data_assunzione = inizio contratto di lavoro
-- 2) tabella hr_eventi: traccia con data assegnazione categoria,
--    impiego (jolly/fisso), premi consegnati, passaggi di livello,
--    inizio contratto — scritti in automatico dall'app
-- 3) whitelist secure_* estesa con 'hr_eventi'
-- ============================================================

ALTER TABLE collaboratori ADD COLUMN IF NOT EXISTS data_assunzione DATE;

CREATE TABLE IF NOT EXISTS hr_eventi (
  id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  collaboratore TEXT NOT NULL,
  tipo TEXT NOT NULL, -- 'assunzione' | 'categoria' | 'impiego' | 'premio' | 'livello' | 'nota'
  descrizione TEXT,
...
CREATE INDEX IF NOT EXISTS idx_hr_eventi_collab ON hr_eventi (collaboratore);
CREATE INDEX IF NOT EXISTS idx_hr_eventi_data ON hr_eventi (data_evento DESC);
ALTER TABLE hr_eventi ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS deny_all_anon ON hr_eventi;
CREATE POLICY deny_all_anon ON hr_eventi FOR ALL TO anon USING (false) WITH CHECK (false);

-- Whitelist estese (identiche a 20260827 + hr_eventi)
...
BEGIN
  ...
  IF p_table NOT IN (
    ...
    'valutazioni', 'punti_eventi', 'hr_eventi'
  ) THEN
