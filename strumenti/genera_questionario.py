...
.opz input{margin:0}.opz label.on-vero{background:#e3f0e8;border-color:var(--ok);font-weight:700}.opz label.on-falso{background:#f8e1de;border-color:var(--no);font-weight:700}.opz label.on-nonso{background:#efe9d9;border-color:var(--oro);font-weight:700}
...
<header><h1>Diario Collaboratori · verifica di tutte le regole</h1><p>Ogni riga dice come il programma si comporta oggi, con un esempio. Chi conosce il regolamento segna <b>Vero</b> se e giusto, <b>Falso</b> se e sbagliato (scrivendo nella nota come dovrebbe essere), <b>Non so</b> se non e di sua competenza. Le risposte restano salvate in questo browser; alla fine <b>Salva le risposte</b> crea il file da rinviare. <b>Stampa</b> produce la versione su carta o PDF, un blocco per pagina.</p></header>
<div class="barra no-stampa"><button class="prim" onclick="salvaRisposte()">Salva le risposte (file da inviare)</button><button onclick="document.getElementById('carica').click()">Carica risposte</button><input type="file" id="carica" accept=".json" style="display:none" onchange="caricaRisposte(this)"><button onclick="window.print()">Stampa / PDF</button><button onclick="azzera()">Azzera</button>''' + ''.join('<a href="#b-%s">%s</a>' % (b[0], E(b[1].split('·')[0].strip())) for b in blocchi) + '''<span class="prog" id="prog"></span></div>
<main>
...
try{applica(JSON.parse(localStorage.getItem(CH)||'null'))}catch(e){}progresso();
</script></body></html>''')
