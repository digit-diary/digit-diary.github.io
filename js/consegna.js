...
async function initConsegnaUI() {
  ...
  if (turno === 'PRESTO') {
    ...
    const rappD = _rapportoCacheGet(domaniStr);
...
// Consegne che un operatore puo' vedere: le sue, quelle destinate a lui o a tutti
// (l'admin anche quelle senza destinatario). Unico filtro per pagina Consegne e Home.
function _consegneVisibiliA(op) {
  return getConsegneReparto().filter(
    (c) => c.operatore === op || _includeOpInCsv(c.destinatario, op) || (isAdmin() && !c.destinatario),
  );
}
function renderConsegne() {
  ...
  el.innerHTML = _cr
    ...
    .map((c) => {
      ...
      // risultare "letta" anche solo ricaricando la pagina o cambiando settore
      if (!isMia && !letto) {
        btns +=
...
// "Fatto" dalla Home: prima renderDashboard() partiva senza aspettare il salvataggio
// e il promemoria restava a video (con doppi click, doppie occorrenze dei ripetitivi)
async function _completaPromemoriaDaHome(id) {
  await completaPromemoria(id);
  renderDashboard();
}
function renderDashboard() {
  ...
  getBudgetReparto().forEach((b) => {
    const spent = _spesoPerBudget(b.nome);
    const ne = _jsArg(b.nome);
    if (b.budget_chf && spent >= b.budget_chf * 0.8) {
      alertH +=
        ...
        '\')"><strong>' +
        escP(b.nome) +
        '</strong></span>: Maison questo mese ' +
        fmtCHF(spent) +
        '/' +
  ...
  if (isVis('rapporto')) {
    if (!rapportiCache[_rapportoKey(oggi)]) {
      // Il settore va fissato prima della risposta: nel frattempo l'utente puo' cambiarlo
      const _repRichiesto = currentReparto;
      secGet('rapporti_giornalieri?data_rapporto=eq.' + oggi + '&reparto_dip=eq.' + _repRichiesto)
        .then((rData) => {
          if (rData && rData.length) {
            const k = _rapportoKey(oggi, _repRichiesto);
            if (!rapportiCache[k]) rapportiCache[k] = {};
            rData.forEach((r) => {
              rapportiCache[k][r.turno] = r;
            });
            if (_repRichiesto === currentReparto) renderDashboard();
          }
        })
        .catch(() => {});
    }
...
function generaReportAnnualeCSV() {
  ...
  for (var m = 0; m < 12; m++) {
    ...
    rows.push([
      ...
      _contaTotaleMalattie(mData, nomeCorrente('Malattia')),
      fmtCHF(
