...
async function initConsegnaUI() {
  ...
  if (!rapportiCache[oggi]) {
    try {
      const rData = await secGet(
        "rapporti_giornalieri?data_rapporto=eq." +
          oggi +
          "&reparto_dip=eq." +
          currentReparto,
      );
  ...
  if (turno === "PRESTO") {
    if (rapp.NOTTE && rapp.NOTTE.sup_note)
      suggerito = rapp.NOTTE.sup_note.trim();
  } else {
    ...
    if (rappD.PRESTO && rappD.PRESTO.sup_note)
      suggerito = rappD.PRESTO.sup_note.trim();
    if (!suggerito && rapp.PRESTO && rapp.PRESTO.sup_note)
      suggerito = rapp.PRESTO.sup_note.trim();
  }
...
function matchCollaboratoreOps(nome, ops) {
  ...
  return (
    ...
    ops.find(
      (n) => n.toLowerCase().includes(nome) || nome.includes(n.toLowerCase()),
    ) ||
    ""
  );
}
async function inviaConsegnaTurno() {
  ...
  try {
    ...
    inviaPush(
      pushDests,
      "Consegna " + turno + " da " + getOperatore(),
      msg.substring(0, 120),
      "consegna",
    );
  } catch (e) {
...
async function segnaConsegnaLetta(id) {
  ...
  try {
    await secPatch("consegne_turno", "id=eq." + id, {
      letto_da: op,
...
function renderConsegne() {
  ...
  var _cr = getConsegneReparto().filter((c) => {
    if (!(
      c.operatore === op ||
      _includeOpInCsv(c.destinatario, op) ||
      (isAdmin() && !c.destinatario)
    ))
      return false;
  ...
  el.innerHTML = _cr
    ...
    .map((c) => {
      ...
      const destLabel =
        c.destinatario && c.destinatario !== "tutti" ? c.destinatario : "Tutti";
      let btns = "";
      if (isMia) {
        if (canEdit) {
          btns +=
          ...
          btns +=
            '<span style="font-size:.82rem;color:var(--muted);font-style:italic">Non modificabile (letta)</span>';
        }
      }
      if (!isMia && !letto) {
      ...
      return (
        ...
        "</span>" +
        (isAlta
          ? '<span style="color:var(--accent);font-size:.82rem;font-weight:700">PRIORITA ALTA</span>'
          : "") +
        (letto
...
async function annullaConsegna(id) {
  ...
  if (!confirm("Annullare l'invio di questa consegna? Verrà eliminata."))
    return;
  try {
...
function aggiornaConsegnaBadge() {
  ...
  const nonLette = getConsegneReparto().filter(
    (c) =>
      !c.letto_da && c.operatore !== op && _includeOpInCsv(c.destinatario, op),
  ).length;
  if (nonLette) {
...
function mostraConsegnaLogin() {
  ...
  mostraNotifBanner(
    "consegna",
    "Consegna da " + ultima.operatore,
    ultima.messaggio.substring(0, 80),
    () => switchPage("consegna"),
  );
  ...
  let html =
    ...
    (isAlta
      ? '<span style="color:var(--accent)"><i class="icx icx-avviso"></i></span> '
      : "") +
    "Consegna dal turno " +
    ...
    " " +
    new Date(ultima.created_at).toLocaleTimeString("it-IT", {
      hour: "2-digit",
      minute: "2-digit",
    }) +
    "</p>";
  if (isAlta)
    html +=
      '<p style="color:var(--accent);font-size:.82rem;font-weight:700;margin-bottom:10px">PRIORITA ALTA</p>';
  html +=
...
function renderDashboard() {
  ...
  function _getDataRemind(p) {
    ...
    if ((p.descrizione || "").includes("il giorno stesso"))
  ...
  if (!todoH)
    todoH =
      '<p style="color:#2c6e49;text-align:center;padding:20px;font-weight:600">Tutto in ordine!</p>';
  ...
  const bdaysDomani = isVis("alert_compleanni")
    ? _getCompleanniProssimi(1).filter((b) => b.giorni === 1)
    : [];
  ...
  if (!domH)
    domH =
      '<p style="color:var(--muted);text-align:center;padding:20px">Niente in programma</p>';
  ...
  if (compEl && isVis("alert_compleanni")) {
    ...
    // Compleanni collaboratori. Il settore si legge dall'anagrafica, non dalle
    // registrazioni: prima chi non aveva mai una riga nel diario spariva da qui
    // pur comparendo nella fascia in cima alla pagina, e i due elenchi non
    // coincidevano. Stesso criterio ovunque, cosi' i compleanni sono uno solo.
    const bdaysCollab = [];
    collaboratoriCache.forEach((c) => {
      // 'attivo' non valorizzato vuol dire attivo: solo un false esplicito esclude
      if (c.attivo === false || !c.data_nascita) return;
      const _repC = c.reparto_dip || "slots";
      if (_repC !== currentReparto && _repC !== "entrambi") return;
  ...
  if (_consRecenti.length) {
    ...
    _consRecenti.forEach((c) => {
      ...
      alertH +=
        ...
        "</span>" +
        (isAlta
          ? '<span style="color:var(--accent);font-size:.82rem;font-weight:700">!</span>'
          : "") +
  ...
  if (!alertH)
    alertH =
      '<p style="color:#2c6e49;text-align:center;padding:20px;font-weight:600">Nessun alert attivo</p>';
  ...
  if (maisonOggiRighe.length) {
    ...
    mEl.innerHTML =
      '<p style="color:var(--muted);text-align:center;padding:20px">Nessun dato Maison oggi</p>';
  }
...
function _renderDashSettimana(_rd, _rm) {
  ...
  const regPerGiorno = sett.map(
    (ds) => _rd.filter((e) => e.data && e.data.startsWith(ds)).length,
  );
  const maisonPerGiorno = sett.map((ds) =>
    _rm
      .filter((r) => r.data_giornata === ds)
      .reduce((s, r) => s + parseFloat(r.costo || 0), 0),
  );
  renderChart(
    ...
    },
    {
      ...
      scales: {
        y: {
          beginAtZero: true,
          position: "left",
          title: { display: true, text: "Registrazioni" },
        },
        y1: {
          beginAtZero: true,
          position: "right",
...
function apriReportConfig(tipo) {
  ...
  var sez = [
    ...
    {
      key: "registrazioni",
      label: "Registrazioni dettagliate",
      checked: false,
    },
  ];
  mc.innerHTML =
...
async function generaReportSettimanale(sez) {
  ...
  const settData = getDatiReparto().filter(
    (e) => e.data >= sett[0] && e.data <= sett[6] + "T23:59:59",
  );
  const settMaison = getMaisonRepartoExpanded().filter(
    (r) => r.data_giornata >= sett[0] && r.data_giornata <= sett[6],
  );
  const settModuli = getModuliReparto().filter(
    (m) => (m.created_at || m.data_modulo || "") >= sett[0],
  );
  const settPM = promemoriaCache.filter(
    (p) =>
      (!p.reparto_dip || p.reparto_dip === currentReparto) &&
      p.data_scadenza >= sett[0] &&
      p.data_scadenza <= sett[6],
  );
  try {
    ...
    doc.text(dalLabel + " · " + alLabel + " · Casino Lugano SA", pw / 2, y, {
      align: "center",
    });
    y += 10;
    doc.setTextColor(0);
    // PANORAMICA
    const totMaison = settMaison.reduce(
      (s, r) => s + parseFloat(r.costo || 0),
      0,
    );
    if (sez.panoramica) {
      const nErr = settData.filter(
        (e) => e.tipo === nomeCorrente("Errore"),
      ).length;
      ...
      const nApprMod = settModuli.filter(
        (m) => m.tipo === "apprezzamento",
      ).length;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(11);
    ...
    if (sez.promemoria && settPM.length) {
      ...
      const pmFatti = settPM.filter((p) => p.completata).length,
        pmScad = settPM.filter(
          (p) => !p.completata && p.data_scadenza <= sett[6],
        ).length;
      doc.setFont("helvetica", "bold");
      doc.setFontSize(11);
      ...
      doc.autoTable({
        ...
        body: settPM.map((p) => [
          ...
          p.assegnato_a,
          p.completata
            ? "Completato" + (p.completata_da ? " da " + p.completata_da : "")
            : "In corso",
        ]),
    ...
    // Spese Extra
    if (sez.spese_extra) {
      const settSE = getSpeseReparto().filter(
        (r) => r.data_spesa >= sett[0] && r.data_spesa <= sett[6],
      );
      if (settSE.length) {
        const totSE = settSE.reduce(
          (s, r) => s + parseFloat(r.importo || 0),
          0,
        );
        if (y + 20 > doc.internal.pageSize.getHeight() - 20) {
          doc.addPage();
    ...
    if (sez.regali) {
      const settReg = getRegaliReparto().filter(
        (r) =>
          (r.data_regalo || "") >= sett[0] && (r.data_regalo || "") <= sett[6],
      );
      if (settReg.length) {
        const totReg = settReg.reduce(
          (s, r) => s + parseFloat(r.importo || 0),
          0,
        );
        if (y + 20 > doc.internal.pageSize.getHeight() - 20) {
        ...
        doc.setFontSize(11);
        doc.text(
          "Regali" + (totReg ? " (CHF " + fmtCHF(totReg) + ")" : ""),
          mx,
          y,
        );
        y += 2;
        doc.autoTable({
          ...
          body: settReg.map((r) => [
            new Date(
              (r.data_regalo || r.created_at) + "T12:00:00",
...
function generaReportSettimanaleCSV() {
  ...
  rows.push(
    [""],
    ["COSTI MAISON"],
    ["Data", "Cliente", "PX", "Costo CHF", "Tipo"],
  );
  settMaison.forEach((r) => {
    rows.push([
  ...
  if (settSE.length) {
    rows.push(
      [""],
      ["SPESE EXTRA"],
      ["Data", "Beneficiario", "Tipo", "Luogo", "CHF"],
    );
    settSE.forEach((r) => {
      rows.push([
        new Date(r.data_spesa + "T12:00:00").toLocaleDateString("it-IT"),
...
function _getDatiAnno(anno) {
  ...
  return {
    data: getDatiReparto().filter(
      (e) => e.data >= start && e.data <= end + "T23:59:59",
    ),
    maison: getMaisonRepartoExpanded().filter(
      (r) => r.data_giornata >= start && r.data_giornata <= end,
    ),
    spese: getSpeseReparto().filter(
      (r) => r.data_spesa >= start && r.data_spesa <= end,
    ),
    moduli: getModuliReparto().filter(
      (m) =>
        (m.created_at || m.data_modulo || "") >= start &&
        (m.created_at || m.data_modulo || "") <= end + "T23:59:59",
    ),
  };
}
  ...
  try {
    ...
    doc.autoTable({
      ...
      body: [
        ...
        ["Malattie", nMal],
        [
          "Apprezzamenti (moduli)",
          d.moduli.filter((m) => m.tipo === "apprezzamento").length,
        ],
        ["Costi Maison totali", "CHF " + fmtCHF(totMaison)],
        ["Spese Extra totali", "CHF " + fmtCHF(totSE)],
        [
          "Totale complessivo (Maison+Extra)",
          "CHF " + fmtCHF(totMaison + totSE),
        ],
        [
          "Allineamenti",
          d.moduli.filter((m) => m.tipo === "allineamento").length,
        ],
...
function generaReportAnnualeCSV() {
  ...
  rows.push(
    [""],
    ["REGISTRAZIONI"],
    ["Data", "Collaboratore", "Tipo", "Descrizione"],
  );
