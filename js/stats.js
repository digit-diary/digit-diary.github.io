...
function renderStatistiche() {
  ...
  const tipiCount = {};
  _ds.forEach((e) => {
    tipiCount[e.tipo] = (tipiCount[e.tipo] || 0) + 1;
  });
  ...
  const tutti = getTuttiTipi(),
    cd = {};
  _ds.forEach((e) => {
    if (!cd[e.nome]) cd[e.nome] = { tot: 0 };
    cd[e.nome].tot++;
    if (e.tipo === _tipoMalStat) {
      cd[e.nome][e.tipo] = (cd[e.nome][e.tipo] || 0) + _contaGiorniMalattia(e);
    } else {
      cd[e.nome][e.tipo] = (cd[e.nome][e.tipo] || 0) + 1;
    }
  });
  const cr = Object.entries(cd).sort((a, b) => b[1].tot - a[1].tot),
    ...
    errSorted.forEach(([n, d]) => {
      ...
      et +=
        ...
        (d.totCHF
          ? fmtCHF(d.totCHF) +
            (d.amm || d.ecc
              ? '<br><span style="font-size:.82rem"><span style="color:#c62828">-' +
                fmtCHF(d.amm) +
...
function renderChart(id, type, data, opts) {
  ...
  if (esistente && esistente.config && esistente.config.type === type && esistente.canvas === el) {
    ...
    if (esistente.data.datasets.length === data.datasets.length) {
      data.datasets.forEach((ds, i) => Object.assign(esistente.data.datasets[i], ds));
    } else {
