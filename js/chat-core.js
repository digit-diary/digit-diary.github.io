...
function _chatGetGroupById(group_id) {
  return chatGroupsCache.find((g) => g.id === group_id);
}
// Helper: trova i membri di un gruppo
function _chatGetGroupMembers(group_id) {
  return chatGroupMembersCache.filter((m) => m.group_id === group_id).map((m) => m.operatore);
}
// Helper: ritorna il legacy gruppo_id se disponibile, altrimenti sintetizzato
function _chatGroupGid(group) {
  if (!group) return null;
  return group.legacy_gid || '__chat_group_' + group.id;
}
// Helper: trova chat_message per id
function _chatFindMsg(id) {
  return chatMessagesCache.find((m) => m.id === id);
}
// Helper: chi ha letto un messaggio
function _chatLetti(message_id) {
  return chatLettiCache.filter((l) => l.message_id === message_id);
}
// Helper: ha letto un operatore?
function _chatHasLetto(message_id, operatore) {
  return chatLettiCache.some((l) => l.message_id === message_id && l.operatore === operatore);
}
// Helper: e' nascosto per un operatore?
function _chatIsHidden(message_id, operatore) {
  return chatHiddenCache.some((h) => h.message_id === message_id && h.operatore === operatore);
}
...
async function _chatPatchMessage(chatMessageId, updates) {
  ...
  if (Object.keys(directUpdates).length > 0) {
    ...
    try {
      await secPatch('chat_messages', 'id=eq.' + chatMessageId, directUpdates);
    } catch (e) {
      console.error('chat_messages patch fallita:', e.message);
      throw e;
    }
...
async function _chatDeleteMessage(chatMessageId) {
  try {
    await secDel('chat_messages', 'id=eq.' + chatMessageId);
  } catch (e) {
    console.error('chat_messages delete fallita:', e.message);
    throw e;
  }
...
async function _chatInsertMessage(opts) {
  ...
  if (isGroup) {
    ...
    if (!group) {
      let tipo = 'custom',
        nome = null;
      if (legacyGid === '__gruppo_slots') {
        tipo = 'slots';
        nome = 'Tutti Slots';
      } else if (legacyGid === '__gruppo_tavoli') {
        tipo = 'tavoli';
        nome = 'Tutti Tavoli';
      } else if (legacyGid === '__gruppo_tutti') {
        tipo = 'tutti';
        nome = 'Tutti';
