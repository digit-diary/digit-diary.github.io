...
// ---- VALUTATORI della scheda annuale (impostazione valutatori_config) ----
async function renderValutatoriUI() {
  const el = document.getElementById('valutatori-list');
  if (!el || !isAdmin()) return;
  if (typeof _valutatoriCarica === 'function' && !window._valutatoriCfg) await _valutatoriCarica();
  const cfg = window._valutatoriCfg || {};
  el.innerHTML = getReparti()
    .map(
      (r) =>
        '<div class="tipo-item"><span class="mini-badge" style="background:' +
        (r.colore || 'var(--accent2)') +
        '">' +
        escP(r.label) +
        '</span><input type="text" id="valutatori-' +
        r.key +
        '" value="' +
        escP((cfg[r.key] || []).join(', ')) +
        '" placeholder="Cognome Nome, Cognome Nome, ..." style="flex:1" onchange="salvaValutatori(\'' +
        r.key +
        '\', this.value)"></div>',
    )
    .join('');
}
async function salvaValutatori(rep, testo) {
  if (!isAdmin()) return;
  const cfg = Object.assign({}, window._valutatoriCfg || {});
  const lista = String(testo || '')
    .split(/[,;\n]+/)
    .map((x) => capitalizzaNome(x.trim()))
    .filter(Boolean);
  // avviso se un nome non e fra i collaboratori ne fra gli operatori (puo essere voluto: es. HR)
  const noti = new Set(
    (collaboratoriCache || [])
      .map((c) => c.nome.toLowerCase())
      .concat((operatoriAuthCache || []).map((o) => o.nome.toLowerCase())),
  );
  const ignoti = lista.filter((n) => !noti.has(n.toLowerCase()));
  if (lista.length) cfg[rep] = lista;
  else delete cfg[rep];
  if (!(await salvaImp('valutatori_config', JSON.stringify(cfg)))) return;
  window._valutatoriCfg = cfg;
  logAzione('Valutatori scheda annuale', repartoLabel(rep) + ': ' + (lista.join(', ') || 'nessuno'));
  toast(
    'Valutatori ' +
      repartoLabel(rep) +
      ' salvati (' +
      lista.length +
      ')' +
      (ignoti.length ? ' · non in anagrafica: ' + ignoti.join(', ') : ''),
    ignoti.length ? 6000 : undefined,
  );
  renderValutatoriUI();
}
