...
function _d(e) {
  const b = atob(e),
    r = [];
  for (let i = 0; i < b.length; i++) r.push(String.fromCharCode(b.charCodeAt(i) ^ _k.charCodeAt(i % _k.length)));
  return r.join('');
}
const SB_URL = _d(_u),
  SB_KEY = _d(_a);
...
const TIPI_DEFAULT = [
  { nome: 'Attività', colore: '#1a4a7a' },
  { nome: 'Richiesta', colore: '#2c6e49' },
  { nome: 'Errore', colore: '#c0392b' },
  { nome: 'Ammonimento Verbale', colore: '#7b2d8b' },
  { nome: 'Malattia', colore: '#1a7a6d' },
  { nome: 'Non Disponibilità', colore: '#5d6d7e' },
  { nome: 'Nota', colore: '#8b6914' },
  { nome: 'Altro', colore: '#8a7d6b' },
];
let tipiPersonalizzati = [],
  coloriOverride = {},
  tipoSelezionato = 'Attività',
  modalEntryId = null,
  modalTipoSel = null,
  datiCache = [],
  pinnedIds = new Set(),
  ...
  charts = {},
  operatoriSalvati = [];
let rapportoMese = new Date().getMonth(),
  rapportoAnno = new Date().getFullYear(),
  rapportoGiornoAperto = null,
  rapportiCache = {},
  noteColleghiCache = [],
  operatoriAuthCache = [];
